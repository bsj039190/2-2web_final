...
$selrec="select * from announce order by no desc limit $frow,$line_page";
$info=mysqli_query($connect, $selrec);
while($rowinfo=mysqli_fetch_array($info))
{
	echo "<tr>";
	echo "<td> $rowinfo[no] </td>";
    echo "<td> <a href='29_detailan.php?title=$rowinfo[title]'> $rowinfo[title] </a></td>";
...
<button><a target="iframe1" href="29_writein.html?board_id=notice">글 쓰기</a></button>